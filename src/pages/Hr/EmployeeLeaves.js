...
import {getAllEmployeeLeaves,deleteLeave} from '../../services/apiLeavePortal';
...
const columns = [
  ...
  { field: "comment", headerName: "Comment", flex:5},
 {
    field: "actions",
    headerName: "Action",
    flex: 0,
    renderCell: (params) => (
      <div style={{ display: 'flex', alignItems: 'center' }}>
       ...
       <IconButton
          color="secondary" onClick={() => handleDelete(params.row._id)}
         >
...
<div className="">
        <button onClick={()=>{
          OpenDialog();
        }}  className="btn btn-info infobtn addleave">Add Leave</button>
      </div>
</div>
      <div>
      <DataGrid
     ...
     <AddLeave open={open} onClose={handleClose} listLeaves={handleListLeaves} employeeId={employeeId} editMode={editMode}  leavevalues={selectedRow} errors={errors} setErrors={setErrors} ></AddLeave>
    </div>
    {openPopUp && (
